...
processors! Currently only the legacy/older stable [RPi](RPi) OS versions 11 and 13  
with GNU/Octave are what PTB supports well. While PTB runs in principle on the  
[RPi](RPi) OS 12, performance is seriously degraded due to known bugs without fixes,  
so [RPi](RPi) OS 12 is not recommended at all! Note that we will soon remove support  
for [RPi](RPi) OS versions older than v13 "Trixie", and you must run the latest [RPi](RPi)  
OS 13 version with all updates applied for a good experience. [RPi](RPi) OS itself  
also provides bundled versions of Psychtoolbox, installable via "sudo apt  
install octave-psychtoolbox-3", and 64-Bit versions of [RPi](RPi) OS would ship such  
...
- Dual-display support via two micro-HDMI connectors up to 4k UHD resolution,  
  ...
  source. [RPi](RPi) OS 13 already ships a modern enough Mesa for this to work  
  out of the box.  
